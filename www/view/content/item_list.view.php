...
<div class="item" id="item<?= ++$i ?>" item-id="<?= $item->get('num_iid') ?>"<?=
    ($p = $item->pic_url(true)) ? " p=\"$p\"" : '' 
?>><?php 
if ($i === 10 || $i === $item_count) { 
?><script> Izheko.lazy_img(); </script><?php } ?>

    <h1><?= $item->type_tag() ?><a href="<?= $item->url() ?>" target="_blank"><?= $item->title() ?></a></h1>
    <h2>
        ...
        <a data-rd="1" class="<?= $item->action_style() ?>" title="<?= $item->action_title() ?>" href="<?= $item->url() ?>" target="_blank"><?= $item->action() ?></a>
    </h2>
    <div>
